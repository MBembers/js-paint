...
let prevX: number;
let prevY: number;
let preserveProportions: boolean = false;
...
function pickColorOk() {
  ...
  let color = (document.getElementById("colorpicker") as HTMLInputElement)
    .value;
  // console.log(color);
  strokeColor = color;
  fillColor = color;
...
function draw(event: MouseEvent) {
  if (is_drawing) {
    ...
    if (mode != "brush") context.beginPath();
    ...
    switch (mode) {
      ...
      case "rect":
        ...
        if (preserveProportions) {
          ...
          context.strokeRect(
            prevX,
            prevY,
            drawWidth >= 0 ? minSize : minSize * -1,
            drawHeight >= 0 ? minSize : minSize * -1
          );
          if (fill)
            context.fillRect(
              prevX,
          ...
          if (fill) context.fillRect(prevX, prevY, drawWidth, drawHeight);
        }
        break;
      ...
      case "line":
        ...
        if (preserveProportions) {
          ...
          else context.lineTo(prevX, prevY + drawHeight);
        } else {
          context.lineTo(getX(event), getY(event));
        }
        break;
...
function Download(url: string) {
  (document.getElementById("my_iframe") as HTMLIFrameElement).src = url;
}
...
function switchProportions(event: Event) {
  let input = event.target as HTMLInputElement;
  preserveProportions = Boolean(input.checked);
}
function switchFill(event: Event) {
  let input = event.target as HTMLInputElement;
  fill = Boolean(input.checked);
}

function clear(save: boolean = false) {
  context.fillStyle = "white";
...
function saveStep() {
  ...
  if (restore_array.length > 100) restore_array.shift();
  else start_index += 1;
}
